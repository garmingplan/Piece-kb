...
def split_by_length(content: str, base_name: str, chunk_size: int = 800) -> List[Dict]:
    """按递归字符切分文档（用于无标题文档）"""
...
def split_large_chunk(chunk_content: str, base_name: str, h2_name: str) -> List[Dict]:
...
def split_by_headings(content: str, base_name: str) -> List[Dict]:
    """
    按二级标题切分文档
    ...
    if not h2_matches:
        return split_by_length(content, base_name)
    ...
    for i, match in enumerate(h2_matches):
        ...
        if len(chunk_content) > 2000:
            sub_chunks = split_large_chunk(chunk_content, base_name, h2_name)
